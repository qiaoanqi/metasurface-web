...
def add_title_shape(slide, text, left, top, width, height, font_size=Pt(28), bold=True, color=None, align=PP_ALIGN.LEFT):
    """添加标题形状"""
    if color is None:
        color = COLOR_PRIMARY
    txBox = slide.shapes.add_textbox(left, top, width, height)
...
def add_body_text(slide, text, left, top, width, height, font_size=Pt(13), bold=False, color=None, align=PP_ALIGN.LEFT):
    """添加正文文本"""
    if color is None:
        color = COLOR_TEXT
    txBox = slide.shapes.add_textbox(left, top, width, height)
...
# ============================================================
slide = prs.slides.add_slide(blank_layout)
set_slide_bg_color(slide, COLOR_BG)
...
table_data = [
    ...
    ["介质材料", "TiO2（锐钛矿）、a-Si（非晶硅）", "光学损耗低、色彩柔和", "透射式显示、生物传感"],
    ["金属材料", "Ag、Al", "共振强度高、色彩饱和度高", "反射式显示、光学防伪"],
    ["衬底材料", "SiO2、Si3N4", "工艺成熟、器件兼容性好", "标准微纳加工工艺"]
]
...
for i, row in enumerate(table_data[1:]):
    ...
    for j, cell in enumerate(row):
        is_bold = (j == 0)
        font_sz = Pt(9) if j == 1 else Pt(10)
        add_body_text(slide, cell,
...
desc_text = ("直观展示纯 TiO2 纳米柱与 FP 腔结构的色域覆盖范围差异" + chr(10) +
...
add_body_text(slide, "[CIE 1931 色品图]" + chr(10) + "标注 TiO2 纳米柱与 FP 腔结构的色域边界",
...
# 保存
# ============================================================
output_path = 'AI超表面结构色智能设计系统_演示文稿_v2.pptx'
