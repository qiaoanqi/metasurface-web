"""train_dual_multi.py - Dual-pillar multi-material ResMLP (4 materials, 8 inputs, 200k samples)"""
import numpy as np, torch, torch.nn as nn, torch.nn.functional as F, pickle, os, time
...
def single_spectrum(d, h, p, angle_deg, mat_name):
    A, B = MATERIALS[mat_name]
    n_550 = A + B / (0.55**2)
    dn_ref = n_550 - 2.0
    ...
    fill_amp = 0.30 + 0.80*fill; loss = np.exp(-0.0006*max(h-600, 0))
    theta = angle_deg*np.pi/180.0; sin2 = np.sin(theta)**2
    ...
    inv_norm_ed = 1.0/np.sqrt(1.0+q_ed**2); inv_norm_md = 1.0/np.sqrt(1.0+q_md**2)
    ed_center = lam_ed+ed_shift; detune_ed = (WL-ed_center)/sigma_ed
    fano_num_ed = q_ed + detune_ed; denom_ed = 1.0 + detune_ed**2
    r_ed_real = fano_num_ed * inv_norm_ed / denom_ed * np.sqrt(ed_amp_a)
    r_ed_imag = -fano_num_ed * detune_ed * inv_norm_ed / denom_ed * np.sqrt(ed_amp_a)
    md_center = lam_md+md_shift; detune_md = (WL-md_center)/sigma_md
    fano_num_md = q_md + detune_md; denom_md = 1.0 + detune_md**2
    r_md_real = fano_num_md * inv_norm_md / denom_md * np.sqrt(md_amp_a)
    r_md_imag = -fano_num_md * detune_md * inv_norm_md / denom_md * np.sqrt(md_amp_a)
    r_real = w_ed*r_ed_real + w_md*r_md_real
    r_imag = w_ed*r_ed_imag + w_md*r_md_imag
    return (r_real**2 + r_imag**2) * fill_amp * loss

def dual_spectrum(d1, h1, d2, h2, p, angle_deg, mat_name):
    return single_spectrum(d1, h1, p, angle_deg, mat_name) + single_spectrum(d2, h2, p, angle_deg, mat_name)
...
# Generate 200k samples (50k per material)
N_PER_MAT = 25000
N_SAMPLES = N_PER_MAT * len(MAT_LIST)
print(f"Generating {N_SAMPLES} dual-pillar samples ({N_PER_MAT} per material)...")
np.random.seed(42)
X_norm = np.zeros((N_SAMPLES, 8), dtype=np.float32)
...
for mat_code, mat_name in enumerate(MAT_LIST):
    ...
    for _ in range(N_PER_MAT):
        d1 = np.random.uniform(60, 267)
        h1 = np.random.uniform(80, 600)
        d2 = np.random.uniform(60, 267)
        h2 = np.random.uniform(80, 600)
        p = max(max(d1, d2)*1.2 + 20, np.random.uniform(200, 600))
        angle = np.random.uniform(0, 80)
        pol = np.random.choice([0, 1])
        spec = dual_spectrum(d1, h1, d2, h2, p, angle, mat_name)
        rgb = spec_to_rgb(spec)
        X_norm[idx] = [(d1-60)/247, (h1-80)/520, (d2-60)/247, (h2-80)/520, (p-200)/400, angle/80, float(pol), float(mat_code)]
        Y_spec[idx] = spec
        Y_rgb[idx] = rgb
        idx += 1
...
# Model: ResMLP_Dual_Multi with 8 inputs
...
class ResMLP_Dual_Multi(nn.Module):
    def __init__(self, in_dim=8, hidden=256, out_dim=81, n_blocks=4):
        super().__init__()
        ...
        self.output = nn.Linear(hidden, out_dim)  # NO Sigmoid - dual spec can be >1
    def forward(self, x):
...
model = ResMLP_Dual_Multi().to(device)
...
save_path = os.path.join(save_dir, "models", "dual_mlp_v3_multi.pt")
os.makedirs(os.path.dirname(save_path), exist_ok=True)

BATCH = 256 if device.type == "cuda" else 128
print(f"Training ResMLP_Dual_Multi (80 epochs, {N_SAMPLES} samples, batch={BATCH})...")
...
# Test
print("\nDual ML-Multi vs Physics comparison:")
test_cases = [
    (200,300,120,200,400,0,"TiO2 (anatase)"),
    (150,150,250,400,500,0,"a-Si (amorphous)"),
    (100,200,200,350,450,0,"Si3N4 (nitride)"),
    (180,380,100,250,420,0,"Al2O3 (sapphire)"),
    (250,450,150,300,500,30,"TiO2 (anatase)"),
]
for d1,h1,d2,h2,p,ang,mat in test_cases:
    mc = MAT_LIST.index(mat)
    x = torch.tensor([[(d1-60)/247,(h1-80)/520,(d2-60)/247,(h2-80)/520,(p-200)/400,ang/80,0.0,float(mc)]], dtype=torch.float32).to(device)
    with torch.no_grad():
        spec_ml = model(x).cpu().squeeze().numpy()
        rgb_ml = spec_to_rgb(spec_ml)
    spec_phys = dual_spectrum(d1,h1,d2,h2,p,ang,mat)
    rgb_phys = spec_to_rgb(spec_phys)
    de = np.sqrt(np.sum((rgb_ml - rgb_phys)**2))
    print(f"  {mat}: D1={d1:.0f} H1={h1:.0f} D2={d2:.0f} H2={h2:.0f} P={p:.0f} | dE={de:.4f}")

data_path = os.path.join(save_dir, "training_data", "dual_200k_v3_multi.pkl")
with open(data_path, "wb") as f:
    pickle.dump({"X": X_norm, "Y_spec": Y_spec, "Y_rgb": Y_rgb}, f)
print(f"\nDone! Model: {save_path}, Data: {data_path}")
