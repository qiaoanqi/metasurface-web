...
def init_ml():
    ...
    try:
        ...
        _path = os.path.join(_dir, "models", "forward_mlp_v3.pt")
        ...
        class ForwardMLP(nn.Module):
            def __init__(self):
                ...
                self.net = nn.Sequential(
                    nn.Linear(5, 512), nn.ReLU(), nn.BatchNorm1d(512),
                    nn.Linear(512, 512), nn.ReLU(), nn.BatchNorm1d(512),
                    nn.Linear(512, 512), nn.ReLU(), nn.BatchNorm1d(512),
                    nn.Linear(512, 81), nn.Sigmoid())
            def forward(self, x):
                return self.net(x)
...
def predict_rgb(d_nm, h_nm, p_nm, angle_deg=0.0, polarization="TE"):
    """ML proxy model: input raw params, output normalized RGB (R,G,B)"""
    if not _ML_AVAILABLE:
        return None
    import torch
    pol_code = 0.0 if polarization.startswith("TE") else 1.0
    with torch.no_grad():
        x = torch.tensor([[(d_nm-50)/300, (h_nm-80)/520, (p_nm-200)/400, angle_deg/80, pol_code]], dtype=torch.float32)
        spec = _ML_FWD(x)
        wl = torch.linspace(380, 780, 81)
        X = torch.trapz(spec * _ML_CIE_X.unsqueeze(0), wl, dim=1)
        Y = torch.trapz(spec * _ML_CIE_Y.unsqueeze(0), wl, dim=1)
        Z = torch.trapz(spec * _ML_CIE_Z.unsqueeze(0), wl, dim=1)
...
def predict_spectrum(d_nm, h_nm, p_nm, angle_deg=0.0, polarization="TE"):
...
def inverse_design_ml(target_rgb, n_steps=300, n_restarts=40):
    ...
    for _ in range(n_restarts):
        ...
        for step in range(n_steps):
            ...
            x = torch.stack([(d_c-50)/300, (h_c-80)/520, (p_c-200)/400,
                           torch.tensor(0.0), torch.tensor(0.0)]).unsqueeze(0)
            spec = _ML_FWD(x)
            wl = torch.linspace(380, 780, 81)
            X = torch.trapz(spec * _ML_CIE_X.unsqueeze(0), wl, dim=1)
            Y = torch.trapz(spec * _ML_CIE_Y.unsqueeze(0), wl, dim=1)
        ...
        with torch.no_grad():
            x = torch.tensor([[(d_f-50)/300, (h_f-80)/520, (p_f-200)/400, 0.0, 0.0]], dtype=torch.float32)
            spec = _ML_FWD(x)
            wl = torch.linspace(380, 780, 81)
            X = torch.trapz(spec * _ML_CIE_X.unsqueeze(0), wl, dim=1)
            Y = torch.trapz(spec * _ML_CIE_Y.unsqueeze(0), wl, dim=1)
