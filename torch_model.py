...
def batch_lorentzian_spectrum(D, H, P, theta=0.0, pol_TE=True, material=None, substrate=None):
    ...
    # CCM effective fill factor: f_eff = f0 + Delta-f (data-driven correction)
    ccm = get_ccm(material or "TiO2 (anatase)", substrate or "SiO2 (fused silica)")
    fill_np = ccm.effective_fill_batch(
        D.detach().numpy().astype(np.float64),
        P.detach().numpy().astype(np.float64)
    )
    fill = torch.from_numpy(fill_np.astype(np.float32)).unsqueeze(1)
